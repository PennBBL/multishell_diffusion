...
# Run topup to calculate correction for field distortion
	#topup --imain=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_b0s.nii.gz --datain=$1 --out=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_topup --fout=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_field --iout=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_topup_iout
# Actually correct field distortion
	#applytopup --imain=$inputnifti --datain=$1 --inindex=1 --topup=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_topup --out=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_topup_applied --method=jac
...
# Run eddy correction. Corrects for Electromagnetic-pulse induced distortions. Most computationally intensive of anything here, has taken >5 hours. More recent eddy correction available in more recent FSL versions	
	#/share/apps/fsl/5.0.9/bin/eddy_openmp --imain=$out/Prestats/topup/${bblIDs}_${SubDate_and_ID}_topup_applied.nii.gz --mask=${topup_mask} --index=index.txt --acqp=$1 --bvecs=$bvec --bvals=$out/Prestats/qa/${bblIDs}_${SubDate_and_ID}_roundedbval.bval --out=$eddy_outdir/${bblIDs}_${SubDate_and_ID}_eddied
