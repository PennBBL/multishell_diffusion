...
if [ $nreps -lt 3 ]; then 
    ...
    if [ $append -eq 1 ]; then 
        echo -e "meanABSrms${subfield}\t-1" >> $resultfile
        echo -e "meanRELrms${subfield}\t-1" >> $resultfile
        echo -e "maxABSrms${subfield}\t-1"  >> $resultfile
        echo -e "maxRELrms${subfield}\t-1"  >> $resultfile
    fi
    exit 1
fi

# --- moco ---
mcflirt -plots -rmsrel -rmsabs -verbose 0 -in $infile -refvol 0 -out $outdir/${inroot}_mc >&/dev/null
meanABSrms=`cat $outdir/${inroot}_mc_abs_mean.rms`
meanRELrms=`cat $outdir/${inroot}_mc_rel_mean.rms`
echo -e "meanABSrms${subfield}\t$meanABSrms" >> $resultfile
echo -e "meanRELrms${subfield}\t$meanRELrms" >> $resultfile
...
echo -e "maxABSrms${subfield}\t$maxABSrms" >> $resultfile
echo -e "maxRELrms${subfield}\t$maxRELrms" >> $resultfile
...
if [ $keep -eq 0 ]; then 
	...
	rm -f $outdir/${inroot}_mc.par
    rm -rf $outdir/${inroot}_mc.mat*
	imrm $outdir/${inroot}_mc
fi
