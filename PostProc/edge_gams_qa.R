...
msFAedges_Age_pvals <- mclapply(msFAedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
ssFAedges_Age_pvals <- mclapply(ssFAedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
msADedges_Age_pvals <- mclapply(msADedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
ssADedges_Age_pvals <- mclapply(ssADedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
msMD_invedges_Age_pvals <- mclapply(msMD_invedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
ssMD_invedges_Age_pvals <- mclapply(ssMD_invedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
msRD_invedges_Age_pvals <- mclapply(msRD_invedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
ssRD_invedges_Age_pvals <- mclapply(ssRD_invedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
ICVFedges_Age_pvals <- mclapply(ICVFedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
oneminODIedges_Age_pvals <- mclapply(oneminODIedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
oneminISOVFedges_Age_pvals <- mclapply(oneminISOVFedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
rtopedges_Age_pvals <- mclapply(rtopedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
rtapedges_Age_pvals <- mclapply(rtapedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Run a gam on each edge, yield p values
rtppedges_Age_pvals <- mclapply(rtppedges, function(x) { summary(gam(formula = x, fx=TRUE,data=df, REML=T))$p.pv[2]},mc.cores = 4)
...
# Summarize sig. edges
paste("This is the final edge count for QA associations,", " chief")
paste("msFA:", length(FDR_msFA_sig))
paste("ssFA:", length(FDR_ssFA_sig))
paste("msAD:", length(FDR_msAD_sig))
paste("ssAD:", length(FDR_ssAD_sig))
