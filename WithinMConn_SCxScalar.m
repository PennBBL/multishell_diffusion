...
for i=1:length(subjects)
    ...
    cd(currD)
	filename=[num2str(subjects(i)),'_',sprintf(date(3).name),'_','ODI_matrixts.csv']
    icvf_path = fullfile('/', 'data','jux','BBL','projects','multishell_diffusion','GroupLevelAnalyses','5_13', 'SCxTS')
	cd(icvf_path)
	path_name = dir([num2str(subjects(i)),'_',sprintf(date(3).name),'_','SCxFA.csv'])
   icvf_mat = csvread((path_name.name),1,1);
	...
	for i=unique_S'
		...
		ICVF_csvcontent(:,1)=win;
		ICVF_csvcontent(:,2)=bw;
		%	  = ([num2str(subjects(i)), ', ' num2str(wb_vec(1)), ', ' num2str(wb_vec(2))])
		 currD = fullfile('/', 'data','jux','BBL','projects','multishell_diffusion','processedData','multishellPipelineFall2017', num2str(subjects(i)))
		cd(currD)		
		date = dir
		currD2 = fullfile('/', 'data','jux','BBL','projects','multishell_diffusion','processedData','multishellPipelineFall2017', num2str(subjects(i)), sprintf(date(3).name), 'tractography')
		cd(currD2)
    		csvwrite('SCxFACon.csv',ICVF_csvcontent);
	end
	...
	%	  = ([num2str(subjects(i)), ', ' num2str(wb_vec(1)), ', ' num2str(wb_vec(2))])
	subjects(i) 	
	csvwrite('AverageSCxFACon.csv',ICVF_csvcontents) 
end
